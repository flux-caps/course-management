export default class FluxMenu extends HTMLElement {

  #shadowRoot;

  static itemSlot = /*html*/`
    <slot name="item">${this.createItemSlotChild().outerHTML}</slot>
  `;

  /**
   * @return {HTMLElement}
   */
  static createItemSlotChild() {
      const menuItem = document.createElement('a');
      menuItem.className = "menu-item";
      menuItem.href = "#";
      menuItem.ariaCurrent = "";
      menuItem.innerHtml = "";
      return menuItem;
  }
  ...
  connectedCallback() {
    fetch('http://localhost:8010/api/flux-menu/edit-students.json')
    .then((response) => response.json())
    .then((items) => {
        items.forEach(item => this.appendItem(this, item))
      }
    );
  }

  /**
   *  @param {FluxMenu} fluxMenu
   *  @param {node} item
   */
  appendItem(fluxMenu, item) {
    const menu = document.getElementById('menu-id-todo');
    let slotItem = FluxMenu.createItemSlotChild();
    slotItem = Object.assign(slotItem, {slot: "item"})

    menu.append(
      Object.assign(slotItem, item)
    )
  }
